...
import { CardMosaicAnimated } from '@elements/card';
...
const ViewPager = ({cases}) => {
  ...
  const { stylesCards, stylesSecondCard, stylesNav, isFinished, showMosaicSlider, classTransitions } = useStartHomeAnimation();

  return (
    <div className="grid grid-cols-2 lg:grid-cols-3 grid-rows-3 md:grid-rows-2 lg:grid-rows-1 gap-0 w-full">
      <div
        className={
          "col-span-1 border border-secondary-10 "
      ...
      <div
        className={
          "row-span-1 col-span-1 relative overflow-hidden border border-secondary-10 " +
          classTransitions.three
        }
      ...
      >
        ...
        {isFinished && (
          <div
            className={`absolute w-full h-full transform ${
              topVisible
                ? 'transition-all ease-in-out -translate-y-[50%] opacity-0'
        ...
        {isFinished && (
          <div
            className={`absolute w-full h-full transform ${
              topVisible
                ? 'transition-all ease-in-out translate-y-0 opacity-100'
                : 'transition-none translate-y-[50%] opacity-0'
            } duration-1000`}
          >
            <Card
              styles={{
        ...
        <div
          className={
            "absolute space-x-4 text-[32px] right-4 md:right-6 lg:right-8 xl:right-11 bottom-4 md:bottom-6 lg:bottom-12 xl:bottom-16 " +
            classTransitions.five
          }
          style={{ opacity: stylesNav.opacity }}
        >
        ...
        <div
          className={classTransitions.five}
          style={{ opacity: stylesNav.opacity }}
        >
          <div className="absolute inset-x-4 md:inset-x-6 lg:inset-x-8 xl:inset-x-11 bottom-4 md:bottom-6 lg:bottom-8 xl:bottom-11 h-1 md:h-2">
            {!isMobile && (
              <animated.div className={`bg-secondary-10 rounded-full`} style={styles} />
            )}
          </div>
        </div>
      </div>
      <div
        className={
          "row-span-2 col-span-2 lg:col-span-1 md:row-span-1 flex overflow-hidden " +
          classTransitions.four
        }
        style={stylesCards}
      >
        ...
        <div className={`flex-1 relative opacity-100 aspect-square`}>
              ...
              {showMosaicSlider && !isMobile ? (
                ...
                <Card
                  styles={{
                    ...
                    bgSecondary:
                      cases[bottomCaseIndex]?.case_of_study.data.attributes.secondaryColor,
                  }}
                  caseId={cases[bottomCaseIndex]?.case_of_study.data.id}
                  description={
                    cases[bottomCaseIndex]?.case_of_study.data.attributes.caseDescription
                  }
                  imageUrl={
                    cases[bottomCaseIndex]?.case_of_study.data.attributes.image.data.attributes.url
                  }
          ...
          {/* top */}
          <div
            className={`absolute h-full w-full transform ${
              topVisible
          ...
          >
            <Card
              ...
              imageUrl={
                cases[topCaseIndex]?.case_of_study.data.attributes.image.data.attributes.url
              }
